...
function testserver(){
  ...
  if(version_compare(PHP_VERSION, '5.6.0', '<')){
    ...
    $err = true;
  }else if(version_compare(PHP_VERSION, '8.3.99', '>')){
    $phpVersion = $sourceAl.'<span class="notify">Необходимо установить PHP не выше версии 8.3. Текущая версия '.PHP_VERSION.'</span>';
    $err = true;
  } else {
    $phpVersion = $sourceOk;
  }
...
function extractZip($file) {
  if (file_exists($file)) {
    ...
    if($res === TRUE){
      ...
      header("Location: index.php?step1=go&agree=ok&id=34181");
      exit();
    }else{
